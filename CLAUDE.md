...
- `test` in `vite.config.ts` — `{ globals: true, environment: "jsdom", setupFiles: ["src/test/setup.ts"], include: ["src/**/*.{test,spec}.{ts,tsx}"], exclude: ["e2e/**", "node_modules/**", "playwright-report/**", "test-results/**"] }` — keeps `e2e/**` out of unit runs. **Note:** `src/test/setup.ts` is **restored** (F1) — `src/` totals 40 source files + 17 test files + 1 setup, and `pnpm test` reports 16 files / 94 tests green (the guard suite adds a 17th test file).
...
| `pnpm test` / `npm run test` | Vitest `jsdom` — `vitest run` | ✅ **16 files / 94 tests green** | Harness restored (F1): `src/test/setup.ts` + BSC-adapted suite (cn / nav / content / site / massDay / monogram / deepLinks / Button / Accordion / SafeImage / Header / BackToTop / ScrollProgress / Layout / useScrollProgress / useScrollSpy) + round-13 guards (`ci-workflow` / `repo-hygiene` / `docs-contract`). |
...
pnpm test               # vitest run — 16 files / 94 tests green
...
```
src/ (40 source + 17 test + 1 setup files — harness restored, 16 files / 94 tests green)
...
- All five gates are green: `pnpm lint` (0) + `pnpm typecheck` (0) + `pnpm test` (16 files / 94 tests) + `pnpm test:e2e` (51/51, BSC) + `pnpm build` (391.57kB); sixth check `pnpm test:e2e:built` 51/51 (built artifact + live host).
...
| 5 | Testing Strategy (16 files / 94 tests green + 51 E2E green BSC; porting checklist completed — historical) | Yes | ✅ |
