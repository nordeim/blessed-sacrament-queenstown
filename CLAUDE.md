...
- `test` in `vite.config.ts` — `{ globals: true, environment: "jsdom", setupFiles: ["src/test/setup.ts"], include: ["src/**/*.{test,spec}.{ts,tsx}"], exclude: ["e2e/**", "node_modules/**", "playwright-report/**", "test-results/**"] }` — keeps `e2e/**` out of unit runs. **Note:** `src/test/setup.ts` is **restored** (F1) — `src/` totals 40 source files + 17 test files + 1 setup, and `pnpm test` reports 17 files / 117 tests green (BSC suite + `docs-contract`/`repo-hygiene`/`ci-workflow` guards).
...
| `pnpm build` / `npm run build` | Production single-file build (`vite build` + `viteSingleFile`) → `dist/index.html` + `dist/images/` | ✅ | `viteSingleFile` inlines JS+CSS only; `publicDir` is copied verbatim — upload both `dist/index.html` + `dist/images/`; verified `391.57kB` |
...
| `pnpm test` / `npm run test` | Vitest `jsdom` — `vitest run` | ✅ **17 files / 117 tests green** | Harness restored (F1): `src/test/setup.ts` + BSC-adapted suite (cn / nav / content / site / massDay / monogram / deepLinks / Button / Accordion / SafeImage / Header / BackToTop / ScrollProgress / Layout / useScrollProgress / useScrollSpy) + round-13 guards (`ci-workflow` / `repo-hygiene` / `docs-contract`). |
...
pnpm test               # vitest run — 17 files / 117 tests green
...
pnpm build              # vite build — singlefile inlines correctly (391.57kB)
```
...
```
src/ (40 source + 17 test + 1 setup files — harness restored, 17 files / 117 tests green)
...
- All five gates are green: `pnpm lint` (0) + `pnpm typecheck` (0) + `pnpm test` (17 files / 117 tests) + `pnpm test:e2e` (51/51, BSC) + `pnpm build` (391.57kB); sixth check `pnpm test:e2e:built` 51/51 (built artifact + live host).
...
| 5 | Testing Strategy (17 files / 117 tests green + 51 E2E green BSC; porting checklist completed — historical) | Yes | ✅ |
