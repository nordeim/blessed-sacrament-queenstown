...
- `types: ["node", "vitest/globals"]` — required for `describe/it/expect` globals once `src/test/setup.ts` is restored (currently missing — see Testing Strategy).
...
- `test` in `vite.config.ts` — `{ globals: true, environment: "jsdom", setupFiles: ["src/test/setup.ts"], include: ["src/**/*.{test,spec}.{ts,tsx}"], exclude: ["e2e/**", "node_modules/**", "playwright-report/**", "test-results/**"] }` — keeps `e2e/**` out of unit runs. **Note:** `src/test/setup.ts` is currently **missing** (no `src/test/` directory — 39 files total in `src/`), so `pnpm test` exits with `No test files found`. Restore the harness as the first porting step (F1) — see Testing Strategy.
...
- Colocation: `components/` for layout primitives, `pages/` for route components, `data/` for typed content, `utils/` for pure helpers (`cn`), `hooks/` for `useScrolled` + `useScrollProgress` (currently 2 — no `useScrollSpy`).
...
| `pnpm build` / `npm run build` | Production single-file build (`vite build` + `viteSingleFile`) → `dist/index.html` + `dist/images/` | ✅ | `viteSingleFile` inlines JS+CSS only; `publicDir` is copied verbatim — upload both `dist/index.html` + `dist/images/`; verified `390.74kB` |
...
| `pnpm test` / `npm run test` | Vitest `jsdom` — `vitest run` | ⚠️ wired, **0 files / 0 tests** — harness missing | `vite.config.ts` expects `setupFiles: ["src/test/setup.ts"]` but `src/test/setup.ts` does not exist (no `src/test/` directory; `src/` is 39 files total with 0 `*.test.*`). Vitest exits `No test files found`. Restore harness first (F1). |
| `pnpm test:watch` | Vitest watch mode (`vitest`) | ⚠️ none | Same harness absence — watches 0 files. |
| `pnpm test:coverage` | Vitest with coverage (`vitest run --coverage`) | ⚠️ 0 | Same. Coverage via `@vitest/coverage-v8` once tests return. |
| `pnpm test:e2e` / `npm run test:e2e` | Playwright `chromium` — `playwright test` (8 specs — 51 tests) | ⚠️ **stale** | Specs still assert **Risen Christ** copy (`91 Toa Payoh Central`, `He is risen`, `Velankanni`, `Toa Payoh NS19`) against a BSC build (1 Commonwealth Drive, Tent of Meeting, Commonwealth EW20). Suite will fail until retargeted. |
| `pnpm test:e2e:built` | Playwright vs the **built artifact** — `playwright test --config=playwright.built.config.ts` (`vite preview :4173` serving `dist/`; set `E2E_BASE_URL` to target the live host instead — webServer is skipped) | ⚠️ same stale 51 | Same Risen copy; catches dev/build divergence (round-9 E2E-L1: singlefile rewrites `/favicon.svg` → `./favicon.svg`). |
...
| `pnpm lint && pnpm typecheck && pnpm build` | **Pre-push gate — 3 green today** | ✅ | Full `lint && typecheck && test && test:e2e && build` is the target once unit + E2E are ported; today `test` is red (0/0) and `test:e2e` is stale until F1/F2. CI mirrors the same. |

> Before documenting a command as available, verify it in `package.json` scripts. The three green gates (`lint`, `typecheck`, `build`) are verified 2026-09-02; unit + E2E are the porting work.
...
Current status: **unit harness missing — 0 files / 0 tests + 51 E2E stale (not green).** `vitest 3.2.6` (jsdom) + `@testing-library/react 16.2.0` + `jsdom 26.1.0` are installed and `vite.config.ts` `test` is configured for `{ globals: true, environment: "jsdom", setupFiles: ["src/test/setup.ts"], include: ["src/**/*.{test,spec}.{ts,tsx}"], exclude: ["e2e/**", "node_modules/**", "playwright-report/**", "test-results/**"] }`, but **`src/test/setup.ts` does not exist** (no `src/test/` directory; `src/` totals 39 source files with zero `*.test.*`). `pnpm test` exits `No test files found` and `pnpm test:watch`/`pnpm test:coverage` inherit the same 0. `playwright 1.55.1` (chromium, 8 specs: smoke 11 + navigation 8 + ministries 4 + give-faq 4 + enhancements 7 + enhancements-round5 6 + enhancements-round7 8 + deep-links 3) is configured but **stale**: every spec still asserts Risen Christ copy (e.g. `He is risen`, `91 Toa Payoh Central`, `Velankanni`, `Toa Payoh NS19`) against the BSC build (1 Commonwealth Drive, *A tent of meeting*, Commonwealth EW20, Corpus Christi, SS.CC).

Run `pnpm test` (unit — currently 0), `pnpm test:watch` (watch — currently none), `pnpm test:coverage` (coverage — currently 0), `pnpm test:e2e` (E2E, `webServer` → `pnpm exec vite --port 5173 --host 127.0.0.1 --strictPort` with `reuseExistingServer: !CI`), `pnpm test:e2e:built` (built-artifact pass via `playwright.built.config.ts` — `vite preview :4173` serving `dist/`; `E2E_BASE_URL` retargets to the live host and skips the webServer), `pnpm test:e2e:ui` (UI mode), `pnpm test:e2e:report` (HTML report). `vite.config.ts` `server.watch.ignored` lists `skills`/`dist`/`coverage`/`src.orig` (plus report dirs).

**Porting checklist for BSC (F1/F2 — do before claiming green):**

- Restore `src/test/setup.ts` (jest-dom + IntersectionObserver mock + scrollTo/scrollIntoView + matchMedia stub) — `vite.config.ts` already references it; without it every `pnpm test` run fails before any BSC assertion runs. Then port `src.orig` pure-helper tests with BSC fixtures — `cn`, `massDay` (simplified `weekdays|saturday|sunday`), `monogram` (SS.CC-aware), `deepLinks` (JSDoc-stripped) and `data/nav`, `data/content`, `data/site`.
...
- Restore `src/test/setup.ts` first — nothing else runs until it exists.
- Port `src.orig` pure-helper tests with BSC fixtures — highest value, lowest churn.
...
Gate for pre-ship (three green today, five after porting):

```bash
pnpm lint               # eslint flat — no warnings (0)
pnpm typecheck          # tsc --noEmit (0)
pnpm test               # vitest run — currently 0 files / 0 tests (harness missing → red until F1)
pnpm test:e2e           # playwright chromium — currently 51 stale (Risen copy → red until retarget)
pnpm test:e2e:built     # playwright vs built artifact (vite preview :4173; E2E_BASE_URL → live) — same 51 stale
pnpm build              # vite build — singlefile inlines correctly (390.74kB)
```
...
- Use existing `shrine-*` tokens before introducing new colors. Tokens 25 colors + 2 shadows (incl. `gold-700`) are the budget — frame the Tent of Meeting / Damien Centre / Wihardja Stations imagery with them, don't add arbitrary `bg-[#...]`.
...
- Do not commit `node_modules/`, `.next/`, `dist/`. `skills/` is **deleted in this worktree** (the previous vendored reference content was a symlink to `~/.pi/agent/skills/` — do not reintroduce it as a committed tree; `eslint.config.js` already ignores and `vite.config.ts` watch-ignores `skills/`). **`src.orig/` is the archived Risen Christ port** — **77 files present, git-tracked** (the previous St Mary lineage was replaced during the BSC port; `src.orig/` now holds the Risen Christ snapshot as reference). Do not import or lint it; `eslint.config.js` ignores and `vite.config.ts` watch-ignores it, but the directory itself is intentionally kept unlike the earlier round-12 prune. Never commit secrets: `docs/ssh-key.txt` was untracked in round 3 (C-1) for exactly this reason — same rule applies.
...
- Keep `docs/prompts.md` for lineage prompts; update when intent shifts. Lineage: Rother Shrine → St Joseph BT → St Mary of the Angels → Risen Christ (`src.orig` — 77 files) → Blessed Sacrament Queenstown (`src` — 39 files).
...
```
src/ (39 source files — 0 tests; src/test/setup.ts missing → pnpm test 0/0 until F1)
  ...
  components/
    ...
    Footer.tsx           # 4-col (parish blurb Tent/SS.CC/Corpus/Damien + Explore/Get involved from footerNav + visit/contact) + divider-weave-thin + BSC copy + 2-social + WhatsApp hotline + sacredHearts + parishUpdates + archdiocese + address 1 Commonwealth Drive + Reception/office hours + phone 6474 0582 / WhatsApp 9170 9133 + copyright
...
public/
  images/ (8)            # hero-church.jpg, chapel-interior.jpg, sanctuary.jpg, rosary-garden.jpg, stained-glass.jpg, parish-hall.jpg, cemetery.jpg, feast.jpg (Vite publicDir → dist/images/ — upload alongside dist/index.html); all images local — CDN keys hero/naveCdn/courtyardCdn now point to local fallbacks (no legacy wikimedia/pexels allowlist); no favicon.svg in this worktree (index.html references none — use ogImage for share)
  _headers               # Cloudflare Pages security headers (HSTS/XCTO/XFO/Referrer-Policy/Permissions-Policy) → dist/_headers
vite.config.ts           # alias @→src + test { globals, jsdom, setupFiles: src/test/setup.ts (missing — see Testing Strategy), include: src/**/*.{test,spec}.{ts,tsx}, exclude: e2e/** } + server.watch.ignored [skills/**, dist/**, playwright-report/**, test-results/**, coverage/**, src.orig/**] + viteSingleFile()
...
index.html               # Google Fonts Fraunces + Source Sans 3; CSP `img-src 'self' data: blob:` only + `object-src 'none'` + `base-uri 'self'` + `frame-src https://www.google.com`; no favicon link + theme-color maroon-950 + full OG (url https://www.bsc.org.sg/ / site_name Blessed Sacrament Church / locale en_SG / image hero-church + alt "Blessed Sacrament Church with its folded blue tent roof at dusk") + twitter summary_large_image + Church JSON-LD (name Blessed Sacrament Church / alternateName [Church of the Blessed Sacrament/BSC Queenstown/圣体堂] / address 1 Commonwealth/149603 / telephone +65 6474 0582 / sameAs facebook + catholic.sg) + canonical https://www.bsc.org.sg/; base description SS.CC Tent of Meeting 1 Commonwealth Drive + viewport; #root + /src/main.tsx
e2e/ (8 specs — 51 tests stale — Risen copy) # smoke + navigation + ministries + give-faq + enhancements + enhancements-round5 + enhancements-round7 + deep-links + helpers.ts — currently asserting Risen Christ (91 Toa Payoh, He is risen, Velankanni, Toa Payoh NS19); retarget to BSC (1 Commonwealth Drive, A tent of meeting, Commonwealth EW20, Corpus Christi, SS.CC, TOMR) as F2
src.orig/ (77 files — Risen Christ snapshot, git-tracked) # App.tsx + index.css + vite-env.d.ts + ci-workflow.test.ts + docs-contract.test.ts + head.test.ts + repo-hygiene.test.ts + security-headers.test.ts + components/* + hooks/* + pages/* + data/* + utils/* + test/setup.ts (the Risen Christ port's full source + test suite — 35 files / 202 tests + 51 E2E at 2026-08-31)
...
- **Extending `@theme` with one-off hex values** — add a named `shrine-*` token or reuse an existing one. Tokens 25+2 shadows (incl. gold-700) are the budget.
...
- Three gates are green today and five after porting: `pnpm lint` (0) + `pnpm typecheck` (0) + `pnpm build` (390.74kB) green now; `pnpm test` (0/0 — harness missing) and `pnpm test:e2e` (51 stale Risen copy) must be retargeted to BSC fixtures and pass before the full `lint && typecheck && test && test:e2e && build` gate is claimed.
...
- `skills` is deleted in this worktree (previously a symlink to `~/.pi/agent/skills/`); `src.orig/` is 77 files of the Risen Christ snapshot (git-tracked). Do not import from either.
...
| 4 | Development Workflow (Env Setup + Build Commands — 3 green / 2 red-stale verified 2026-09-02) | Yes | ✅ |
| 5 | Testing Strategy (wired but 0/0 — harness missing src/test/setup.ts + 51 E2E stale Risen copy; BSC porting checklist) | Yes | ✅ |
| 6 | Code Quality Standards (Lint + Type Safety — Priest email only no phone + Styling incl. gold-700) | Yes | ✅ |
| 7 | Git & Version Control (branching + Conventional Commits + CI Node 24/pnpm 11 + HashRouter deploy + src.orig 77 Risen tracked + skills deleted) | Yes | ✅ |
...
| 10 | Project-Specific Standards (Architecture 39 source no tests tree + Data ownership 8 interfaces/11 images all local + Routing 17/7/9 + File Org + Design System 25+2 incl. gold-700) | Yes | ✅ |
| 11 | Success Metrics (10 pages + 7 aliases + 9 anchors + BSC content from data/* + tokens 25+2 incl. gold-700 + Header solid logic + WhatsApp/sacredHearts/no UEN) | — | ✅ |
| 12 | System Integration (tools + skills deleted note) | — | ✅ |
...
| 19 | Risen Christ snapshot `src.orig/` 77 files (historical — 35/202 + 51 as of 2026-08-31; now the BSC worktree's `src.orig/`) | — | ✅ (historical) |
